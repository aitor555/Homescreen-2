var RSS = function() {
    ...
    return class {
        constructor(t, i, n = {}) {
            this.version = "1.3.0", this.target = t, this.urls = [].concat(i), this.html = [], this.options = {
                ssl: !0,
                ...
                entryTemplate: '<li><a href="{url}" class="rssLink" target="_blank"><span class="rssTitle">{title} <span class="rssSmallText">{author}</span></span> <span class="rssText">{shortBodyPlain}</span> <span class="rssSmallText">{date}</span></li></a>',
                tokens: {},
                outputMode: "json",
        ...
        render() {
            return new Promise(async (t, e) => {
                try {
                    ...
                    this.feed = t.responseData.feed, this.entries = t.responseData.feed.entries
                } catch (t) {
                ...
                if (this.target.append(i.layout), 0 !== i.entries.length) {
                    ...
                    }(i.layout, "entries") ? i.layout : i.layout.querySelector("entries");
        ...
        _load() {
            const t = `${`http${this.options.ssl?"s":""}`}://${this.options.host}`,
                e = {
                    support: this.options.support,
                    version: this.version,
                    q: this.urls.map(t => encodeURIComponent(t)).join(",")
                };
            this.options.offsetStart && this.options.offsetEnd && (this.options.limit = this.options.offsetEnd), null !== this.options.limit && (e.num = this.options.limit), null !== this.options.key && (e.key = this.options.key), null !== this.options.encoding && (e.encoding = this.options.encoding), this.options.order && (e.order = this.options.order);
            const i = `${t}?${Object.keys(e).map(t=>`${t}=${e[t]}`).join("&")}`;
        ...
        _generateHTMLForEntries() {
            ...
            return this.entries.forEach((e, i) => {
                const n = this.options.offsetStart,
                    s = this.options.offsetEnd;
                let o;
                n && s ? i >= n && i <= s && this._isRelevant(e, t.entries) && (o = this._evaluateStringForEntry(this.options.entryTemplate, e), t.entries.push(o)) : this._isRelevant(e, t.entries) && (o = this._evaluateStringForEntry(this.options.entryTemplate, e), t.entries.push(o))
            }), this.options.entryTemplate ? t.layout = this._wrapContent(this.options.layoutTemplate.replace("{entries}", "<entries></entries>")) : t.layout = this._wrapContent("<div><entries></entries></div>"), t
        }
        _isRelevant(t, e) {
            const i = this._getTokenMap(t);
            return !this.options.filter || (!this.options.filterLimit || this.options.filterLimit !== e.length) && this.options.filter(t, i)
        }
        _evaluateStringForEntry(t, e) {
            var i = t;
            return (t.match(/(\{.*?\})/g) || []).forEach(t => {
        ...
        _getFormattedDate(t) {
            ...
            if ("undefined" != typeof moment) {
                ...
                return (e = e.locale ? e.locale(this.options.dateLocale) : e.lang(this.options.dateLocale)).format(this.options.dateFormat)
            }
            return t
        }
        _getTokenMap(e) {
            ...
            return {
                ...
                bodyPlain: function(e) {
                    for (var i = e.content.replace(/<script[\\r\\\s\S]*<\/script>/gim, "").replace(/<\/?[^>]+>/gi, ""), n = 0; n < t.length; n++) i = i.replace(new RegExp("<" + t[n], "gi"), "");
                    return i
                }(e),
